...
public class LiveInterviewController {
    ...
    // =========================================================================
    // 8b. NEW: Live Broadcasting for AI Interview — lookup by interviewSessionId
    //     GET /api/live-interview/by-interview-session/{interviewSessionId}
    //     Security: MANAGER (recruiter dashboard) or the assigned JOBSEEKER.
    //
    //     Returns the auto-created broadcast session (token NOT included —
    //     candidates get their token via /candidate/{applicationId} and
    //     recruiters via the WebSocket invite / this summary + /join/{token}
    //     flow is not needed here since the recruiter already has the token
    //     from the invite). Includes the mirrored AI interview status and the
    //     current question so the dashboard can render everything in one call.
    // =========================================================================
    @GetMapping("/by-interview-session/{interviewSessionId:\\d+}")
    public ResponseEntity<LiveSessionSummaryResponse> getBroadcastByInterviewSession(
            @PathVariable Long interviewSessionId,
            @AuthenticationPrincipal UserDetails userDetails) {

        log.debug("GET /api/live-interview/by-interview-session/{} called by: {}",
                interviewSessionId, userDetails != null ? userDetails.getUsername() : "UNKNOWN");

        LiveSessionSummaryResponse response = liveService.getBroadcastByInterviewSession(
                interviewSessionId, userDetails.getUsername());
    ...
    // =========================================================================
    // 8c. NEW: Recruiter dashboard — list all live AI-interview broadcasts
    //     currently WAITING or ACTIVE for the calling recruiter.
    //     GET /api/live-interview/active-broadcasts
    //     Security: MANAGER only
    // =========================================================================
    @GetMapping("/active-broadcasts")
    public ResponseEntity<List<LiveSessionSummaryResponse>> getActiveBroadcasts(
            @AuthenticationPrincipal UserDetails userDetails) {

        log.debug("GET /api/live-interview/active-broadcasts called by: {}",
                userDetails != null ? userDetails.getUsername() : "UNKNOWN");

        List<LiveSessionSummaryResponse> sessions =
                liveService.getActiveBroadcastsForRecruiter(userDetails.getUsername());

        log.debug("✓ Returned {} active broadcast(s)", sessions.size());
    ...
    // =========================================================================
    // 9.  NEW: AI Interview Monitoring — full violation log for a session
    //     GET /api/live-interview/{id}/violations
    //     Security: MANAGER only (enforced in SecurityConfig + service layer
    //     re-checks that the caller is this session's own recruiter).
    //
    //     Returns every phone/face/multi-face/noise/tab-switch/camera/mic
    //     violation recorded during the interview, with timestamps and
    //     severity levels, plus a per-severity summary count.
    // =========================================================================
    @GetMapping("/{id:\\d+}/violations")
    public ResponseEntity<ViolationLogResponse> getViolationLog(
            @PathVariable Long id,
            @AuthenticationPrincipal UserDetails userDetails) {

        log.debug("GET /api/live-interview/{}/violations called by: {}", id,
                userDetails != null ? userDetails.getUsername() : "UNKNOWN");

        ViolationLogResponse response = liveService.getViolationLog(id, userDetails.getUsername());

        log.debug("✓ Returned {} violation(s) for liveSessionId={}", response.getTotalCount(), id);
