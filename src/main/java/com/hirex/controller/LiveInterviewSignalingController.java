 ...
 * │  → Server     /app/live/mic                MicStatusMessage              │
 * ├──────────────────────────────────────────────────────────────────────────┤
...
public class LiveInterviewSignalingController {
    ...
    // =========================================================================
    // 5b. Microphone status — sent by EITHER participant when their mic is
    //     muted/unmuted. Broadcast to /topic/mic/{sessionId}, mirroring the
    //     camera status handler above.
    // =========================================================================
    /**
     * /app/live/mic
     */
    @MessageMapping("/live/mic")
    public void handleMicStatus(@Valid @Payload MicStatusMessage msg, Principal principal) {
        requirePrincipal(principal);
        liveService.updateMicStatus(msg, principal.getName());
    }

    // =========================================================================
    // 6.  Proctoring violation report — candidate → server → both parties
    //     NEW: AI Interview Monitoring (phone/face/multi-face/noise/tab-switch)
    // =========================================================================
    /**
     * /app/live/violation
     *
     * Sent by the candidate's browser the moment a client-side detector
     * (blazeface, coco-ssd, WebAudio noise meter, visibilitychange listener,
     * camera/mic track state) fires. The service persists it and broadcasts
     * to /topic/violation/{liveSessionId} so both the candidate's own live
     * warning banner and the recruiter's live violation feed update instantly.
     */
    @MessageMapping("/live/violation")
    public void handleViolation(@Valid @Payload ViolationReportMessage msg, Principal principal) {
        requirePrincipal(principal);
        liveService.recordViolation(msg, principal.getName());
    }
