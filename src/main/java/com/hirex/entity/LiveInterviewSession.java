...
public class LiveInterviewSession {
    ...
    /**
     * Distinguishes a session auto-created the moment the AI interview starts
     * (AI_INTERVIEW) from one a recruiter manually creates via the "Live Camera
     * Monitoring" picker UI (MANUAL). Kept purely for reporting/filtering —
     * the WebRTC signaling path is identical for both.
     */
    @Enumerated(EnumType.STRING)
    @Column(name = "session_origin", nullable = false)
    private LiveSessionOrigin sessionOrigin = LiveSessionOrigin.MANUAL;

    /**
     * The AI question currently being asked, mirrored here so the recruiter's
     * live-broadcast panel can display it without a separate round-trip to the
     * AI-interview endpoints. Updated by AIInterviewService each time a new
     * question is served to the candidate.
     */
    ...
    public LiveSessionOrigin getSessionOrigin() { return sessionOrigin; }
    public void setSessionOrigin(LiveSessionOrigin sessionOrigin) { this.sessionOrigin = sessionOrigin; }
