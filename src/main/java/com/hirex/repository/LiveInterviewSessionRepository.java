...
public interface LiveInterviewSessionRepository extends JpaRepository<LiveInterviewSession, Long> {
     ...
     * Used by the recruiter dashboard to list every candidate currently
     * broadcasting (or waiting to broadcast) their AI interview, so the
     * recruiter doesn't have to know the interviewSessionId up front.
     */
    List<LiveInterviewSession> findByRecruiterIdAndSessionStatusInOrderByCreatedAtDesc(
            Long recruiterId, List<LiveSessionStatus> statuses);
}